    ...
    <div id="address-sidebar">
      ...
      <div class="form-group mb-2">
        <select name="location[id_country]" id="inputCountry" data-old="{{ $register->country ?? null }}"
          ...
          <option value="{{ $value->id }}" @if (isset($register->country) && $register->country == $value->descr->name)
            selected
            @endif
            >
            {{ $value->descr->name }}
      ...
      <div class="form-group mb-2">
        <input type="text" class="form-control" name="location[complement]" value="{{ $register->complement ?? null }}"
          id="inputAdditionaInfo" placeholder="{{ __('Complemento'); }}">
      </div>
...
<script type="text/javascript"
  src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBlYRX4JcJLzJQ4S8lnVZy6FdvipuszGKY">
</script>

...
"use strict";
/*!***********************************************************!*\
  !*** ./resources/assets/site/js/assistance/assistance.js ***!
  \***********************************************************/
...
var Assistance = /*#__PURE__*/function () {
  function Assistance() {
  ...
  _createClass(Assistance, [{
    ...
    value: function map() {
      ...
      $('#maps .list-item').each(function (i, item) {
        var gps = $(item).data('gps');
        ...
        if (gps) {
          ...
          var marker = new google.maps.Marker({
            id: i,
            position: position,
            icon: new google.maps.MarkerImage(base_url + '/assets/site/img/marker.png', new google.maps.Size(45, 45), new google.maps.Point(0, 0), new google.maps.Point(26, 56)),
            map: _map
          });
          ...
          google.maps.event.addListener(marker, 'click', function (marker, i) {
            return function () {
              var company = $('#gps-' + i).data('company');
              var template = '<div class="infobox">...</div>';
              if ($('#gps-' + i).length) template = '<div class="infobox"><div class="infobox-data">' + '<div class="infobox-title">' + company.name + '</div>' + '<div class="infobox-address">' + company.full_address + // ((company.address_number && company.address_number != '-') ? ', ' + company.address_number : '') +
              // ((company.address_district && company.address_district != '-') ? ', ' + company.address_district : '') +
              // ((company.cep && company.cep != '-') ? ', ' + company.cep : '') +
              '</div>' + (company.email && company.email != '-' ? '<a class="infobox-contact" href="mailto:' + company.email + '"><i class="fas fa-envelope"></i><span>' + company.email + '</span></a>' : '') + (company.phone && company.phone != '-' ? '<a class="infobox-contact" href="tel:' + company.phone + '"><i class="fas fa-phone-alt"></i><span>' + company.phone + '</span></a>' : '') + '</div><div class="infobox-buttons"><a class="common-button" href="https://www.google.com/maps/search/?api=1&query=' + company.gps_latitude + ',' + company.gps_longitude + '" target="_blank"><i class="fas fa-map-marker-alt"></i><span>' + (i18n['on_the_map'] || 'Abrir Mapa') + '</span></a>' + '<a class="common-button dark-button" href="https://www.google.com/maps/dir/?api=1&destination=' + company.gps_latitude + ',' + company.gps_longitude + '" target="_blank"><i class="fas fa-route"></i><span>' + (i18n['trace_route'] || 'Traçar Rota') + '</span></a>' + '</div></div>';
              if (infobox) infobox.close();
              infobox.setContent(template);
              infobox.open(_map, marker);
              $('.infoTrending').removeClass('show');
              setTimeout(function () {
                _map.panTo(infobox.getPosition());

                _map.panBy(0, -200);

                infobox.setOptions({
                  'pixelOffset': new google.maps.Size($('.infoTrending').width() / 2 * -1 - 4, -75)
                });
                $('.infoTrending').addClass('show');
              }, 10);
              setTimeout(function () {
                new google.maps.event.trigger(_map, 'dragend');
              }, 200);
            };
          }(marker, i));
        } else {
      ...
      new MarkerClusterer(_map, markers.filter(function (el) {
        return el;
      }), {
        maxZoom: 16,
        styles: [{
      ...
      _map.fitBounds(bounds); //-----LISTENERS E EVENTOS
      ...
      var autocomplete = new google.maps.places.Autocomplete(document.getElementById('searchPlaces')); //Quando escolhe um lugar na busca, atualiza country, state e city
      ...
      $('.map-form').on('submit', function (e) {
        ...
        $.ajax({
          ...
          success: function success(response) {
            ...
            if (response.status) {
              if ($('.result-box .map-box:hidden').length) $('.view-box button:first-child').trigger('click');
              $('html,body').animate({
                scrollTop: $('#gmaps').offset().top - 30
              }, 1000);
              setTimeout(function () {
                var search_bounds = new google.maps.LatLngBounds();
                response.data.map(function (location) {
                ...
                _map.fitBounds(search_bounds);
              }, 100);
            } else {
              Swal.fire('Oops!', response.message || 'Não foi possível executar esta ação', 'warning'); // Se não encontrar nada, mostra todos
    ...
    key: "updateList",
    value: function updateList(markers) {
      var marker_ids = [];

      for (var i = markers.length, bounds = _map.getBounds(); i--;) {
    ...
    key: "updateForm",
    value: function updateForm(location) {
      $('#maps').find('[name=city]').val(location.city.long_name);
      $('#maps').find('[name=state]').val(location.state.long_name);
      $('#maps').find('[name=country]').val(slugify__WEBPACK_IMPORTED_MODULE_0___default()(location.country.long_name, {
        lower: true
      })).trigger('change');
    }
  }]);
